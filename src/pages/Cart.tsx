    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">O Seu Carrinho</h1>
          <p className="text-slate-500 font-medium">Tem {MOCK_CART.length} itens prontos para checkout.</p>
        </div>
        <Link to="/" className="text-slate-500 hover:text-orange-600 font-bold text-sm transition-colors flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-6">
          <AnimatePresence>
            {MOCK_CART.map((item) => (
              <motion.div 
                ...
                className="bg-white rounded-3xl p-6 flex items-center space-x-6 border border-slate-100 shadow-sm relative group overflow-hidden"
              >
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex justify-between items-end">
                    <div className="flex items-center bg-slate-100 rounded-xl p-0.5">
                      <button className="w-8 h-8 flex items-center justify-center font-bold text-slate-500 hover:text-orange-600 text-sm">-</button>
                      <div className="w-10 text-center font-black text-slate-800 text-xs">{item.quantity}</div>
                      <button className="w-8 h-8 flex items-center justify-center font-bold text-slate-500 hover:text-orange-600 text-sm">+</button>
                    </div>
                    <p className="font-black text-slate-900">{(item.price * item.quantity).toLocaleString()} Kz</p>
                  </div>
                </div>
          ...
          {MOCK_CART.length === 0 && (
            <div className="py-24 text-center space-y-4">
              ...
              <p className="text-slate-500 font-bold">O seu carrinho está vazio.</p>
              <Link to="/" className="inline-block bg-orange-600 text-white px-8 py-3 rounded-full font-bold">Explorar Produtos</Link>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-900 rounded-3xl p-8 text-white sticky top-24 space-y-8">
            ...
            <div className="space-y-4">
              <Link 
                to="/checkout"
                className="w-full bg-orange-600 hover:bg-orange-700 py-4 rounded-2xl font-bold flex items-center justify-center space-x-3 transition-all shadow-xl shadow-orange-600/20 group"
              >
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex items-start space-x-3">
              ...
              <p className="text-[10px] text-slate-400 leading-relaxed font-medium">
                Ao finalizar, a plataforma entrará em contacto com o produtor para recolha e entrega ao domicílio.
              </p>
            </div>
          </div>
        </div>
